...
// Problem 1 - Make a new array of all the companies with employees running in the 5k
let companies = runners.map(function(item) {
    return item.company_name;
});
console.log(companies); 

// Problem 2 - Add up all the donations of less than $50 into a new array and log the total of those donations
let smallDonors = runners.filter(function(item) {
    return item.donation < 50;
});
let smallDonorTotal = smallDonors.reduce(function(acc, val) {
    return acc + val.donation;
}, 0);
console.log(smallDonorTotal);

// Problem 3 - Make an array of all the government employees
let govEmployees = runners.filter(function(item) {
    return item.email.slice(-1) === "v";
});
console.log(govEmployees);